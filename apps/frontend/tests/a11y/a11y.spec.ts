...
const defaultUserEmail = "john.doe-a11y@ts.berlin";
...
testWithoutSplashScreen.describe("Accessibility - Auth Flow", () => {
	testWithoutSplashScreen(
		"Invite completion form and main page after login should be accessible",
		async ({ browserName, page }) => {
			// Skip Firefox due to CI environment issues with headless mode
			testWithoutSplashScreen.skip(
				browserName === "firefox",
			...
			const registerA11yResults = await new AxeBuilder({
				page: page1,
			}).analyze();
			...
			const mainPageA11yResults = await new AxeBuilder({
				page: page1,
			}).analyze();
	...
	testWithoutSplashScreen.afterAll("Cleanup user", async () => {
