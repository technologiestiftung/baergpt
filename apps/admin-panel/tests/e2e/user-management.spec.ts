	...
	testWithUser.describe("Ban / Unban user", () => {
		testWithUser(
			...
			async ({ page, testUser }) => {
				...
				// Confirm
				await banDialog
					.getByRole("button", { name: "Benutzer sperren" })
					.click();
		...
		testWithUser(
			...
			async ({ page, testUser }) => {
				...
				// Open edit modal – "Account freischalten" button only appears for banned users
				const editDialog = await openEditModal(page, testUser.email);
				await editDialog
					.getByRole("button", { name: "Account freischalten" })
					.click();

				const restoreDialog = page.getByRole("dialog", {
					name: "Account freischalten",
				});
				await expect(restoreDialog).toBeVisible();
				await expect(restoreDialog.getByText(testUser.email)).toBeVisible();

				await restoreDialog
					.getByRole("button", { name: "Freischalten" })
					.click();
