...
import { register } from "../fixtures/register.ts";
// @ts-expect-error k6 needs the .ts extension
import { getRegistrationEmail } from "../fixtures/constants.ts";
...
export default async function () {
	const registerResponse = await register(
		getRegistrationEmail(__VU, __ITER),
		`123456`,
	);
	check(registerResponse, {
		"register should return a session": (response) => "session" in response,
	});
}
