...
-- STORAGE (MANUALLY ADDED) START --
CREATE POLICY "Authenticated users can upload a new document." ON storage.objects FOR INSERT TO authenticated
WITH
    CHECK (
        bucket_id = 'documents'
        AND (
...
CREATE POLICY "Users can delete objects where their user ID is in the path" ON storage.objects FOR DELETE TO authenticated USING (
    bucket_id = 'documents'
    AND owner_id = (
        (
            SELECT
