...
END IF;
    ...
    -- Ban affected non-admin users indefinitely (pure deactivation, no deletion).
WITH affected AS (
UPDATE auth.users
SET banned_until = '2099-01-01 00:00:00+00'
WHERE (banned_until IS NULL OR banned_until < now())  -- currently active
  AND lower(split_part(email, '@', 2)) = v_domain
  AND id NOT IN (SELECT user_id FROM public.application_admins)
    RETURNING id
    )
...
COMMENT ON FUNCTION public.deactivate_allowed_domain (TEXT) IS 'Deactivates a domain and bans affected non-admin users via auth.users.banned_until.';
