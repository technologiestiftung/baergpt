...
BEGIN
	...
	SELECT
		...
		WHERE (processed_document_chunks.owned_by_user_id IS NULL
			OR processed_document_chunks.owned_by_user_id = user_id)
		AND(use_all = TRUE
...
BEGIN
	...
	SELECT
		...
		WHERE 
		(processed_document_summaries.owned_by_user_id IS NULL
			OR processed_document_summaries.owned_by_user_id = user_id)
		AND(use_all = TRUE
...
BEGIN
	RETURN query WITH chunk_winners AS(
		SELECT
			...
			cw.similarity
		FROM
			match_document_chunks(embedding, match_threshold, chunk_limit, num_probes_chunks, user_id, use_all, allowed_processed_document_ids) AS cw
),
summary_winners AS(
	SELECT
		NULL AS chunk_id,
		...
		sw.similarity
	FROM
		match_summaries(embedding, match_threshold, summary_limit, num_probes_summaries, user_id, use_all, allowed_processed_document_ids) AS sw
),
all_winners AS(
	SELECT
