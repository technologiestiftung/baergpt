...
BEGIN
	...
	SELECT
		...
		WHERE
		  ...
		  OR
		  (search_type = 'private_folder' AND (processed_document_chunks.owned_by_user_id = user_id) AND (allowed_folder_id = processed_document_chunks.folder_id))
		  OR
		  (search_type = 'public_only' AND (processed_document_chunks.owned_by_user_id IS NULL)))
		  AND
		  (processed_document_chunks.chunk_jina_embedding <#> embedding) * -1 > match_threshold
...
BEGIN
	...
	SELECT
		...
		WHERE 
		  ...
		  OR
		  (search_type = 'private_folder' AND (processed_document_summaries.owned_by_user_id = user_id) AND (allowed_folder_id = processed_document_summaries.folder_id))
		  OR
		  (search_type = 'public_only' AND (processed_document_summaries.owned_by_user_id IS NULL)))
		  AND
		  (processed_document_summaries.summary_jina_embedding <#> embedding) * -1 > match_threshold
			ORDER BY
...
BEGIN
	RETURN query WITH chunk_winners AS(
		SELECT
			...
			cw.similarity
		FROM
			match_jina_document_chunks(embedding, match_threshold, chunk_limit, num_probes_chunks, user_id, search_type, allowed_processed_document_ids, allowed_folder_id) AS cw
),
summary_winners AS(
	SELECT
		NULL AS chunk_id,
		...
		sw.similarity
	FROM
		match_jina_summaries(embedding, match_threshold, summary_limit, num_probes_summaries, user_id, search_type, allowed_processed_document_ids, allowed_folder_id) AS sw
),
all_winners AS(
	SELECT
