...
DROP FUNCTION public.match_summaries;
...
DROP FUNCTION public.match_summaries_and_chunks;
...
BEGIN
	RETURN query WITH chunk_winners AS(
		SELECT
			...
			cw.similarity
		FROM
			match_document_chunks(embedding, match_threshold, chunk_limit, num_probes_chunks, user_id, search_type, allowed_processed_document_ids, allowed_folder_id) AS cw
),
summary_winners AS(
	SELECT
		NULL AS chunk_id,
		...
		sw.similarity
	FROM
		match_summaries(embedding, match_threshold, summary_limit, num_probes_summaries, user_id, search_type, allowed_processed_document_ids, allowed_folder_id) AS sw
),
all_winners AS(
	SELECT
