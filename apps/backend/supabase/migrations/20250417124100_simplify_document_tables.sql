...
) returns TABLE (id INTEGER, document_id INTEGER, content TEXT, similarity DOUBLE PRECISION) language plpgsql AS $$
BEGIN
...
) returns TABLE (id INTEGER, document_id INTEGER, summary TEXT, similarity DOUBLE PRECISION) language plpgsql AS $$
BEGIN
  EXECUTE format('SET LOCAL ivfflat.probes = %s', num_probes);
  RETURN QUERY
