...
const MOCK_EXTRACTION_RESULT = {
	parsedPages: [{ content: "page content", tokenCount: 10, pageCount: 1 }],
	checksum: "abc123",
	fileSize: 1234,
	numPages: 1,
};
...
describe("POST /documents/process – captureError is called for every error case", () => {
	...
	describe("summarize() errors", () => {
		...
		let compressToTokenLimitSpy: ReturnType<typeof vi.spyOn>;
		let generateJinaEmbeddingSpy: ReturnType<typeof vi.spyOn>;
		...
		beforeEach(() => {
			...
			compressToTokenLimitSpy = vi.spyOn(
				GenerationService.prototype,
				"compressToTokenLimit",
			);
			generateJinaEmbeddingSpy = vi.spyOn(
		...
		it("calls captureError when compressToTokenLimit throws", async () => {
			const givenError = new Error("Given Error");
			getSummaryInputSpy.mockResolvedValueOnce("some summary input");
			generateSummarySpy.mockResolvedValueOnce("some summary");
			generateOneSentenceSummarySpy.mockResolvedValueOnce("some short summary");
			compressToTokenLimitSpy.mockRejectedValueOnce(givenError);
		...
		it("calls captureError when generateJinaEmbedding throws", async () => {
			const givenError = new Error("Given Error");
			getSummaryInputSpy.mockResolvedValueOnce("some summary input");
			generateSummarySpy.mockResolvedValueOnce("some summary");
			generateOneSentenceSummarySpy.mockResolvedValueOnce("some short summary");
			compressToTokenLimitSpy.mockResolvedValueOnce("some compressed summary");
			generateJinaEmbeddingSpy.mockRejectedValueOnce(givenError);
		...
		it("calls captureError when generateTags throws", async () => {
			...
			generateOneSentenceSummarySpy.mockResolvedValueOnce("some short summary");
			compressToTokenLimitSpy.mockResolvedValueOnce("some compressed summary");
			generateJinaEmbeddingSpy.mockResolvedValueOnce({ embedding: [0] });
	...
	describe("batchEmbed() errors", () => {
		...
		let generateJinaBatchEmbeddingsSpy: ReturnType<typeof vi.spyOn>;
		...
		beforeEach(() => {
			...
			generateJinaBatchEmbeddingsSpy = vi.spyOn(
				EmbeddingService.prototype,
				"generateJinaBatchEmbeddings",
			);
		...
		it("calls captureError when generateJinaBatchEmbeddings throws", async () => {
			const givenError = new Error("Given Error");
			generateJinaBatchEmbeddingsSpy.mockRejectedValueOnce(givenError);
